...
There is one opt-in exception, and it is entirely under your control:
...
**Where it lives.** All of the above is stored locally on your device using the operating system's standard app storage. Sensitive collections (journal, cravings, thoughts, intentions, etc.) are written through Android's **Keystore-encrypted shared preferences** so they remain encrypted at rest. On most modern Android devices the encryption key is held in a hardware-backed secure element (TEE or StrongBox); on older or lower-end hardware Android may keep the key in a software-backed Keystore. Either way the key is managed by the OS and never leaves the Keystore boundary into Journey Forward's own memory.

**What we collect.** Nothing. None of this information leaves your device. There is no account to sign up for, no profile to create on a server, and no copy of your data held by us, by Google, by Journey Forward, or by any third party.
...
Journey Forward does **not** request: internet, camera, location, contacts, SMS, phone state, or calendar.
...
For transparency, Journey Forward is built with the following open-source Flutter packages. They run **on your device only** and transmit none of your data:
...
None of these packages is network-capable in this build. The full list is in [`pubspec.yaml`](pubspec.yaml).
...
**Shawn Fourie / Journey Forward**
Email: shawn@journeyforward.app

---

*Journey Forward is a free sobriety-support app that works fully offline — it has no internet permission and never connects to any server. We will never sell your data because we don't have your data.*
